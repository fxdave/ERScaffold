...
class Generator {
    ...
    static prepareGit() {
        return git
            ...
            .then(() => {
                return git.rollbackToFirstCommit()
            })
    }
    ...
    static processRequest(template) {
        ...
        if (template.meta.creates) {
            // create a file
            let fileName = template.meta.creates.fileName
            let content = template.template_out
            fsp.createFile(fileName, content)
        }

        if (template.meta.extends) {
            // modify the file
            let fileName = template.meta.extends.fileName
            let content = template.template_out
            let place = template.meta.extends.place
            let section = template.meta.extends.section

            fsp.modifyFile(fileName, content, old => {
                if(place == "replace")
                return old.replace(section,content)
                if(place == "after")
                old.indexOf()
            })
        }
