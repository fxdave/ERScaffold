...
class PackController extends Controller {
    ...
    async listPackages (e, model) {
        try {
            ...
            let options = await this.packHandler.getOptionsForEntities(this.model.getEntities())

            return options
                .map(({ entity, packs }) => new PackageListItem(entity.id, entity.name, packs))

        } catch (error) {
            return {success: false, msg: 'Sorry couldn\'t fetch the pack, details:' + error}
        }
    }
