...
class TemplateReader {
    ...
    async getTemplate(packName, templateRelativeURL, data) {
        let templateFileURL = path.join(Config.PACKS_FOLDER_URL, packName, templateRelativeURL)
        let templateFileContent = await this.fsWrapper.getFileContent(templateFileURL)
        ...
        let templateSettings = new TemplateSettings(templateMode,path,section,place)

        // getting the dependencies
        let dependencies = []
        if(meta.depends_on) {
            dependencies = meta.depends_on.map(dependency => {               
                let dependencyTemplateURL = path.join(path.dirname(templateRelativeURL), dependency.template)
                let dependencyTemplateData = dependency.data
                return this.getTemplate(packName, dependencyTemplateURL, dependencyTemplateData)
            })
        }
